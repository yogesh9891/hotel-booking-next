...
export default function Faq() {
    ...
    const [faqArr, setFaqArr] = useState([
        {
            heading: 'How do I make a reservation on your website?',
    ...
    const [sideArr, setSideArr] = useState([
        {
            item: 'Staff and Security',
        },
        {
            item: 'Additional Services',
        },
        {
            item: 'Payment and Check in',
        },
        {
            item: 'Common queries',
        },
                    ...
                    <div className="col-lg-4 col-md-4 col-sm-4">
                        {/* <div className={style.main_content}>
                            <div className={style.image}>
                                <Image src={faq} alt='' fill />
                            </div>

                            <div className={style.content}>
                                <h6 className={style.title}>FAQ’s</h6>
                                <p className={style.para}>So if you're feel stuck, confused, or just need
                                    some extra help, don't hesitate to reach out at
                                    support <Link href='#'>wabisabi@gmail.com</Link></p>
                            </div>
                        </div> */}
                        <h6 className={style.faq_head}>Browse by topic</h6>
                        <ul className={style.sidebar}>
                            {
                                sideArr && sideArr?.length && sideArr?.map((ele, i) => (
                                    <li className={`${style.items} ${activeIndex === i ? `${style.active}` : ''}`} onClick={() => handleActive(i)} key={i}>
                                        {ele?.item}
                                    </li>
                                ))
                            }
